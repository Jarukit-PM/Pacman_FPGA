...
(
    ...
    output wire [7:0] sseg,             //7-segment display output(0-9)
    output wire [3:0] an                //7-segment display anode output(4-bit)
);
    ...
    //score signals
    wire [13:0] score;                          //score of the game
    wire new_score;                             //new score signal
    ...
    //Change speed here
    wire [25:0] speed_offset_blue,speed_offset_red;                   
    assign speed_offset_blue = 2800000; 
    assign speed_offset_red = 3400000; 

    //Color definitions
    wire [11:0] bg_rgb, walls_rgb, pacman_rgb , ghost_blue_rgb, ghost_red_rgb , hearts_rgb, stars_rgb, score_rgb, gameover_rgb;

    //On signals
    wire walls_on, pacman_on, ghost_blue_on, ghost_red_on, hearts_on, stars_on, score_on,gameover_on;
   ...
   assign pacman_right = btnR && game_en;
    ...
    //Instantiate Star
    Stars Stars_unit (.clk(clk), .rst(rst), .pacman_x(pacman_x), .pacman_y(pacman_y), .direction(direction), 
                                            .x(x), .y(y), .stars_on(stars_on), .color_data(stars_rgb), 
                                            .score(score), .new_score(new_score));

    // //Instantiate Score_display
    // Score_display Score_display_unit (.clk(clk), .rst(rst), .new_score(new_score), .score(score), .x(x), .y(y), 
    //                                     .sseg(sseg), .an(an), .score_on(score_on), .color_data(score_rgb));

    // instantiate Gameover_display
    ...
    always @*
        ...
        else if(ghost_blue_on && game_en)
            rgb_next = ghost_blue_rgb;
        
        else if(ghost_red_on && game_en)
            rgb_next = ghost_red_rgb;
        
        else if(hearts_on)
            rgb_next = hearts_rgb;
        
        else if(stars_on && game_en)
            rgb_next = stars_rgb;
