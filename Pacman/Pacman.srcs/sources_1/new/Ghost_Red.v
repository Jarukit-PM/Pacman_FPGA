...
(
    ...
    output wire [9:0] red_x, red_y,          //Red ghost's current location within display area
    output reg ghost_red_on,                //ghost red on signal
...
/***********************************************************************************/
//sprite location registers(Red ghost)
reg [9:0] s_x_reg, s_x_next;
reg [9:0] s_y_reg, s_y_next;

//initialize sprite location registers(top right corner of display area)
...
reg [1:0] dir_reg, dir_next;        //direction register
...
assign col = (dir_reg == LEFT && ghost_red_area) ? T_W - 1 - (x - s_x_reg) : 
             (dir_reg == RIGHT && ghost_red_area) ? x - s_x_reg : 
             (dir_reg == UP && ghost_red_area) ? x - s_x_reg : 
             (dir_reg == DOWN && ghost_red_area) ? x - s_x_reg : 0;

assign row = (dir_reg == LEFT && ghost_red_area) ? y - s_y_reg :
            (dir_reg == RIGHT && ghost_red_area) ? y - s_y_reg :
            (dir_reg == UP && ghost_red_area) ? y - s_y_reg :
            (dir_reg == DOWN && ghost_red_area) ? y - s_y_reg : 0;

//Red ghost rom
wire [11:0] color_data_Red_ghost;
Red_Ghost_rom Red_ghost_unit (.clk(clk), .row(row), .col(col), .color_data(color_data_Red_ghost));

//check if pixel is within display area
wire ghost_red_area;
assign ghost_red_area = (x >= s_x_reg && x < s_x_reg + T_W && y >= s_y_reg && y < s_y_reg + T_H) ? 1 : 0;

//assign output
assign red_x = s_x_reg;
assign red_y = s_y_reg;
